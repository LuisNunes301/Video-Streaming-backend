...
public class ProcessVideoUseCaseImpl implements ProcessVideoUseCase {
  ...
  public void execute(VideoUploadedEvent event) {
    ...
    if (video.getStatus() == VideoStatus.PROCESSING || video.getStatus() == VideoStatus.READY) {
    ...
    try {
      ...
      String playlistKey =
          videoTranscoder.transcodeToHls(video.getId(), video.getObjectKey(), metadata.width());

      var preview = previewGenerator.generate(video.getId(), video.getObjectKey());
