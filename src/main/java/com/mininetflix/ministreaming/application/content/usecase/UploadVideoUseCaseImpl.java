...
public class UploadVideoUseCaseImpl implements UploadVideoUseCase {
  ...
  public UploadVideoOutput execute(UploadVideoInput input) {
    ...
    VideoContent video = VideoContent.create(id, input.title(), input.category(), objectKey);

    catalogRepository.save(video);

    eventPublisher.publish(new VideoUploadedEvent(video.getId(), video.getObjectKey()));
