...
public class GetTrendingVideosUseCaseImpl implements GetTrendingVideosUseCase {
  ...
  public List<TrendingVideoResponse> execute() {
    ...
    return statisticsRepository.findTop20ByOrderByViewsDescCompletedViewsDesc().stream()
        .map(this::toResponse)
        .filter(Objects::nonNull)
        .toList();
  }

  private TrendingVideoResponse toResponse(VideoStatisticsEntity statistics) {

    VideoContent video = videoRepository.findById(statistics.getVideoId()).orElse(null);

    if (video == null || !video.isActive()) {
      return null;
    }

    String thumbnailUrl = videoStorageService.generatePublicUrl(video.getThumbnailKey());
