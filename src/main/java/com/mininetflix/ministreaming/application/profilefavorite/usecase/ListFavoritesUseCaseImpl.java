...
public class ListFavoritesUseCaseImpl implements ListFavoritesUseCase {
  ...
  @Override
  public List<FavoriteResponse> execute(UUID userId) {
    ...
    return repository.findByProfile(profileId).stream()
        .map(
            favorite ->
                videoRepository
