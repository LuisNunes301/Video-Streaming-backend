    ...
    try {
      ...
      runCommand(
          List.of(
              "ffmpeg",
              "-i",
              framesDir.getAbsolutePath() + "/frame_%03d.jpg",
              "-filter_complex",
              "tile=" + cols + "x" + rows,
      ...
      thumbnailStorageService.upload(thumbnailKey, thumbnail);

      thumbnailStorageService.upload(spriteKey, sprite);

      thumbnailStorageService.upload(vttKey, vtt);
