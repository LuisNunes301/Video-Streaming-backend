...
public class UserProfileRepositoryAdapter implements UserProfileRepository {
  ...
  public UserProfile save(UserProfile profile) {
    ...
    return jpaRepository.save(UserProfileEntity.fromDomain(profile)).toDomain();
  }

  @Override
  public Optional<UserProfile> findById(UUID profileId) {

    return jpaRepository.findById(profileId).map(UserProfileEntity::toDomain);
  }

  @Override
  public Optional<UserProfile> findByUserId(UUID userId) {

    return jpaRepository.findByUserId(userId).map(UserProfileEntity::toDomain);
  }
