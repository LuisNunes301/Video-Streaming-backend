...
public class PlaybackRepositoryAdapter implements PlaybackRepository {
  ...
  public Optional<PlaybackState> findByUserAndContent(String userId, String contentId) {
    ...
    return jpaRepository
        .findByUserIdAndContentId(UUID.fromString(userId), UUID.fromString(contentId))
        .map(this::toDomain);
  }
  ...
  public List<PlaybackState> findByUserAndNotCompleted(String userId) {
    ...
    return jpaRepository.findByUserIdAndCompletedFalse(UUID.fromString(userId)).stream()
        .map(this::toDomain)
        .toList();
  }
  ...
  public void save(PlaybackState state) {
    ...
    Optional<PlaybackEntity> existing =
        jpaRepository.findByUserIdAndContentId(
            UUID.fromString(state.getUserId()), UUID.fromString(state.getContentId()));
    ...
    entity.setUpdatedAt(
        state.getLastUpdated() != null
            ? LocalDateTime.ofInstant(state.getLastUpdated(), ZoneId.systemDefault())
            : LocalDateTime.now());
  ...
  private PlaybackState toDomain(PlaybackEntity entity) {
    ...
    PlaybackState state =
        new PlaybackState(entity.getUserId().toString(), entity.getContentId().toString());
    ...
    state.setLastUpdated(entity.getUpdatedAt().atZone(ZoneId.systemDefault()).toInstant());
