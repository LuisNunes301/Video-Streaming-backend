...
public class AuthControllerIntegrationTest {
  ...
  void shouldRegisterUserSuccessfully() throws Exception {
    ...
    RegisterRequest request = new RegisterRequest("Luis", "luis@email.com", "123456");
    ...
    verify(registerUserUseCase).execute(any(RegisterUserInput.class));
  }

  @Test
  void shouldLoginSuccessfully() throws Exception {

    AuthRequest request = new AuthRequest("luis@email.com", "123456");

    when(authenticateUserUseCase.execute(any()))
        .thenReturn(new AuthenticateUserOutput("jwt-token"));
    ...
    verify(authenticateUserUseCase).execute(any());
  }

  @Test
  void shouldReturnCurrentUser() throws Exception {

    GetCurrentUserOutput output =
        new GetCurrentUserOutput(UUID.randomUUID(), "Luis", "luis@email.com");

    when(getCurrentUserUseCase.execute("user-id")).thenReturn(output);

    mockMvc
        .perform(get("/auth/me").with(user("user-id")))
        .andExpect(status().isOk())
