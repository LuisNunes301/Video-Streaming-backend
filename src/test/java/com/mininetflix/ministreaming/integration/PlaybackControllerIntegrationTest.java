...
public class PlaybackControllerIntegrationTest {
  ...
  @MockitoBean private StartPlaybackUseCase startPlaybackUseCase;

  @MockitoBean private SavePlaybackProgressUseCase savePlaybackProgressUseCase;

  @MockitoBean private GetPlaybackProgressUseCase getPlaybackProgressUseCase;

  @MockitoBean private GetContinueWatchingUseCase getContinueWatchingUseCase;

  @Test
  void shouldStartPlayback() throws Exception {

    StartPlaybackOutput output = new StartPlaybackOutput("/videos/master.m3u8", 0.0);

    when(startPlaybackUseCase.execute("user-id", "video-id")).thenReturn(output);

    mockMvc
        .perform(get("/playback/start/video-id").with(user("user-id")))
        .andExpect(status().isOk());

    verify(startPlaybackUseCase).execute("user-id", "video-id");
  }
  ...
  void shouldSaveProgress() throws Exception {
    ...
    verify(savePlaybackProgressUseCase).execute(any());
  }

  @Test
  void shouldReturnContinueWatchingList() throws Exception {

    when(getContinueWatchingUseCase.execute("user-id")).thenReturn(List.of());

    mockMvc.perform(get("/playback/continue").with(user("user-id"))).andExpect(status().isOk());

    verify(getContinueWatchingUseCase).execute("user-id");
  }
  ...
  void shouldReturnPlaybackProgress() throws Exception {
    ...
    when(getPlaybackProgressUseCase.execute("user-id", "video-id")).thenReturn(Optional.of(state));

    mockMvc.perform(get("/playback/video-id").with(user("user-id"))).andExpect(status().isOk());
  }

  @Test
  void shouldReturn404WhenProgressDoesNotExist() throws Exception {

    when(getPlaybackProgressUseCase.execute("user-id", "video-id")).thenReturn(Optional.empty());

    mockMvc
        .perform(get("/playback/video-id").with(user("user-id")))
